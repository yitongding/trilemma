...
export const TrilemmaCard: FC<TrilemmaCardProps> = ({ trilemma }) => {
  ...
  const coords: Record<string, { x: number; y: number; labelY: number; labelX: number }> = {
  ...
  const handleVertexClick = (id: string) => {
    if (selectedIds.includes(id)) {
      ...
      const previouslySacrificed = trilemma.vertices.find(v => !selectedIds.includes(v.id))!.id;
  ...
  const activeTradeOff = trilemma.tradeOffs.find(
    (t) =>
      (t.selectedVertices[0] === selectedIds[0] && t.selectedVertices[1] === selectedIds[1]) ||
      (t.selectedVertices[0] === selectedIds[1] && t.selectedVertices[1] === selectedIds[0])
  ) as TradeOff;

  const sacrificedVertex = trilemma.vertices.find((v) => isSacrificed(v.id)) as Vertex;
      ...
      <div style={{
        ...
        <div style={{ width: '100%', textAlign: 'left', marginBottom: '16px' }}>
          <span style={{
            ...
            {trilemma.category === 'economics' && '📈 经济与金融'}
            {trilemma.category === 'politics' && '⚖️ 政治与全球化'}
            {trilemma.category === 'technology' && '🛡️ 现代科技'}
            {trilemma.category === 'life' && '☕ 趣味与生活'}
          </span>
          <h2 style={{
              ...
              return (
                ...
                >
                  ...
                  <circle
                    ...
                    cy={coord.y}
                    r={4}
                    fill={selected ? '#ffffff' : sacrificed ? '#ef4444' : 'rgba(255, 255, 255, 0.4)'}
                  />

                  {/* Red indicator cross over the sacrificed node */}
                  {sacrificed && (
                    <g transform={`translate(${coord.x}, ${coord.y})`}>
                      <line x1="-6" y1="-6" x2="6" y2="6" stroke="#ef4444" strokeWidth="2.5px" />
                      <line x1="6" y1="-6" x2="-6" y2="6" stroke="#ef4444" strokeWidth="2.5px" />
                    </g>
                  )}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.55 }}>
                必须放弃<strong>【{sacrificedVertex?.name}】</strong>。{sacrificedVertex?.details}
              </p>
            </div>
            ...
            <div style={{
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.55 }}>
                {activeTradeOff?.realWorldImpact}
              </p>
            </div>
